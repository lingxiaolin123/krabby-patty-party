...
#Directions:Use the left or right arrow to move spongebob to avoid the bombs sea shells; Catch the crabby patties and avoid the bombsto advance to the nex level
#Catch the clocks to add time
...
while not game.over:#while loops runs until game is over
    ...
    if game.score<8 and game.time<1:
        game.over = True
        lose.draw()
...
while not game.over and game.score<=0 or game.time<1 :
    ...
    if keys.Pressed[K_SPACE]:
        game.over=True
        game.quit()#added the quit to end the game
...
#game.over = False  ''' #why did you declare all these variables again?  
while not game.over:
    game.processInput()
    game.setBackground(bk)
    bk.draw()
    sb.draw()
    game.drawText("LEVEL TWO - Catch 12 Krabbie Patties to move on to next level",250,5)
    game.drawText("AVOID PATRICK AND THE BOMBS OR YOU WILL LOSE POINTS",250,30)
...
while not game.over and game.score>=12:
    ...
    game.update(30)
game.over = False#added this to keep game going
#level3
kp = []
for index in range(30):
    kp.append( Image( "krabbypatty1.png",game))
...
#THIS DIDNT WORK
    ...
    if game.score<12 and game.time<1:
        game.over = True
        lose.draw()
